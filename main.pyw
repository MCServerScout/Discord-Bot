...
# button to reverse the sort method
@interactions.component_callback("reverse")
async def reverse(ctx: interactions.ComponentContext):
    try:
        org = ctx.message
        ...
        msg = await ctx.send(
            embed=messageLib.standardEmbed(
                title="Sort",
                description="Reversing the sort method...",
                color=BLUE,
            ),
            ephemeral=True,
        )
        ...
        logger.print(f"[main.reverse] reverse called")

        # find the sort method
        for i in range(len(pipelineCP)):
            if "$sort" in pipelineCP[i]:
                sortDict = pipelineCP[i]["$sort"]
                sortMethod = list(sortDict.items())[0]
                sortDict[sortMethod[0]] = -sortMethod[1]
                pipelineCP[i] = {"$sort": sortDict}
                break
        ...
        if "403|Forbidden" in str(err):
            ...
            return
        logger.error(f"[main.reverse] {err}")
        logger.print(f"[main.reverse] Full traceback: {traceback.format_exc()}")
        await ctx.send(
            embed=messageLib.standardEmbed(
                title="Error",
                description="An error occurred while trying to reverse the sort method",
                color=RED,
            ),
            ephemeral=True,
        )
